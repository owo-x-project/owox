...
`owox` v0 は後続 plugin / integration を載せるための最小 extension point を持つ。

## 根拠

v0 で汎用 plugin UI 実行基盤まで作ると Terminal Workspace / Git / Editor / responsive UI の MVP が肥大化する。
...
- 後続 plugin / integration が接続するための manifest、command、backend hook の設計余地がある。
- v0 実装が plugin 固有 domain を内蔵せずに済む。
