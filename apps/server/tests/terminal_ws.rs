...
#[tokio::test]
async fn ws_term_input_writes_to_pty() {
    let temp = tempfile::tempdir().unwrap();
    let repo = make_repo(temp.path(), "repo");
    let data_dir = temp.path().join(".owox-data");
    std::fs::create_dir_all(&data_dir).unwrap();
    ...
    let input = {
        ...
        env.project_id = Some("prj_repo".to_string());
        env.session_id = Some(session_id.clone());
        env.to_msgpack().unwrap()
    };
    ...
    while tokio::time::Instant::now() < deadline {
        ...
        let env = WsEnvelope::from_msgpack(&bytes).expect("decode envelope");
        if env.event_type == EventType::TermOutput {
            let data = env.payload["data"].as_str().unwrap_or("");
    ...
    let _ = ws.close(None).await;
    assert!(saw_ping, "expected terminal input to echo through the PTY");
}
